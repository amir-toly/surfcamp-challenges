...
def better_refrain(lyrics, options = {vibrato: 0, number_of_times: 1})
  ...
  unless options[:vibrato]
    options[:vibrato] = 0
  end

  unless options[:number_of_times]
    options[:number_of_times] = 1
  end

  # no need for strong, since nil is an equivalent to false

  refrain(lyrics, options[:number_of_times], options[:vibrato], options[:strong])
end
